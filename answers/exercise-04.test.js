  ...
  const studentsFullName = (id, arr) => {
    ...
    arr.find(student => {
      if (student.id === id) {
        ...
        const fullName = student.first_name.concat(' ', student.last_name);
...
it('should return true if there are any employees whose credit card type is maestro', () => {
  const employees = [
    {
      id: 1,
      first_name: 'Karoline',
  ...
  // const callBack = employees =>
  //   employees.map(employee => employee.credit_card_type === 'maestro');
  // console.log('callback', callBack);

  const checkForMaestro = employees => {
    // iterate through employees array
    // find if cc = maestro
    // if true, return true
    // else return false
    // employees.reduce((accArray, employee) => {
    //   if (employee.credit_card_type === 'maestro') {
    //     console.log('cc maestro', employee.credit_card_type);
    //     return [...accArray, true];
    //   }
    //   console.log('cc', employee.credit_card_type);
    //   console.log('accArray', accArray);
    //   return [...accArray, false];
    // }, true);
    // };
    // employees.find(employee => employee.credit_card_type === 'maestro');
  };

  // console.log('final', checkForMaestro(employees));

  expect(checkForMaestro(employees)).toEqual(true);
});
